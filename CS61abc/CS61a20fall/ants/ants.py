...
class Insect:
    ...
    # repr() 用于获取对象的官方字符串表示，适合调试和记录对象状态。
    def __repr__(self):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while current_place is not beehive:
            if self.min_range <= n <= self.max_range:
                # print("DEBUG: n =", n)  
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # 创建了原列表 self.place.bees 的一个浅拷贝，也就是一个全新的列表
        # 去除[:],则使用self.place.bees：直接使用原列表，没有创建副本
        if self.armor - amount <= 0:
            sum_damage += self.damage
        Ant.reduce_armor(self, amount)
        for bee in copy:
            Insect.reduce_armor(bee, sum_damage)
        # END Problem 5
...
class HungryAnt(Ant):
    ...
    # OVERRIDE CLASS ATTRIBUTES HERE
    # BEGIN Problem 6
    time_to_digest = 3 # the number of turns that it takes a HungryAnt to digest
    implemented = True   # Change to True to view in the GUI
    # END Problem 6
    ...
    def eat_bee(self, bee):
        ...
        if bee:
            ...
            Insect.remove_from(bee, self.place)
...
class WallAnt(Ant):
    ...
    def __init__(self):
        ...
        self.armor = 4 # `armor` should not be a class attribute
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        "*** YOUR CODE HERE ***"
        super().__init__(armor)
        self.is_impostor = self.identifier
        QueenAnt.identifier += 1
        # END Problem EC
    ...
    def action(self, gamestate):
        ...
        if self.is_impostor != 0:
            ...
            while current_place is not None:
                if current_place.ant is not None and not current_place.ant.buff:
                    current_place.ant.damage *= 2
                    current_place.ant.buff = True
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        Ant.reduce_armor(self, amount)
        if self.armor <= 0 and self.is_impostor == 0:
            bees_win()
        # END Problem EC
