...
def announce_highest(who, last_score=0, running_high=0):
    ...
    #         print("DEBUG: running_high:", running_high)
    #         if increase > running_high:
    #             print(increase, 'point(s)! The most yet for Player', who)
    ...
    """"""
    # # 在python中
    # # you aren't normally allowed to modify variables defined in parent frames
    # # 如果给变量赋值,interpreter会认为你在这scope中创建了一个新变量
    # # 在我的错误例子中
    # def say(score0,score1):
    ...
    #         return announce_highest(who, score1, running_high)
    # return say
    """UnboundLocalError: cannot access local variable 'running_high' where it is not associated with a value """
...
def make_averaged(original_function, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 8
    def new_func(*args):# that returns the average value of ORIGINAL_FUNCTION when called.
        sum = 0
        for _ in range(0,trials_count):
            sum += original_function(*args)
        return sum/trials_count
    return new_func # Return a function
    "*** YOUR CODE HERE ***"
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    n=0
    max=0
    for num_of_rolls in range(10,0,-1):
        res=make_averaged(roll_dice,trials_count)(num_of_rolls,dice)
        # make_averaged(roll_dice,trials_count)  返回的是new_func(*args)函数
        # new_func(*args)接受两个参数,返回平均分
        if res>=max:
            max=res
            n=num_of_rolls
    return n
