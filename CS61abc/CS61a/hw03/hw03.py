...
def quadratic(x, a, b, c):
    ...
    if lower_bound(x)<=-b/2/a<=upper_bound(x):
        return interval(min(m(-2*a/b),m(lower_bound(x)),m(upper_bound(x))),
                max(m(-b/2/a),m(lower_bound(x)),m(upper_bound(x))))
    return interval(min(m(lower_bound(x)),m(upper_bound(x))),
            max(m(lower_bound(x)),m(upper_bound(x))))
    "*** YOUR CODE HERE ***"
